...
def _render_mitre_lines(rules: Iterable[QuarantineRule]) -> list[str]:
    ...
    for technique, count in counter.most_common(TOP_MITRE_PER_CATEGORY):
        out.append(f"- [`{technique}`](https://attack.mitre.org/techniques/{technique}/) " f"— {count:,} rule{'s' if count != 1 else ''}")
...
def render_readme(rules: list[QuarantineRule]) -> str:
    ...
    lines.append("---")
    lines.append("")
    lines.append("_Generated by `scripts/build_quarantine_index.py`. " f"Source: {len(rules):,} YAML rules in this directory._")
    lines.append("")
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"wrote {README_PATH.relative_to(REPO_ROOT)} " f"({len(rules):,} rules indexed, {len(rendered):,} bytes).")
    return 0
