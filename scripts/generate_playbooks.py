...
def investigate(sid: str, name: str, focus: str | None = None) -> dict[str, Any]:
    ...
    if focus:
        params["focus"] = focus
    return step(sid, name, "investigate", params=params, on_failure="continue", timeout=180)


def notify(
    sid: str, name: str, channel: str, message: str, service_env: str | None = None, webhook_env: str | None = None
) -> dict[str, Any]:
...
def ticket(sid: str, name: str, priority: str, title: str, queue: str = "soc") -> dict[str, Any]:
    return step(
        sid,
        name,
        "create_ticket",
...
def close_case(sid: str, name: str, resolution: str = "false_positive", condition: dict[str, Any] | None = None) -> dict[str, Any]:
    return step(
        sid,
        name,
        "close_case",
...
def http(
    sid: str, name: str, url: str, method: str = "POST", body_template: str | None = None, headers_env: str | None = None
) -> dict[str, Any]:
...
def condition(
    sid: str, name: str, field: str, op: str, value: Any, next_true: str | None = None, next_false: str | None = None
) -> dict[str, Any]:
...
def gate_human_approval(
    sid: str, on_oncall_field: str = "context.approved_by_oncall", next_true: str | None = None, next_false: str | None = None
) -> dict[str, Any]:
    ...
    return [
        make_playbook(
            ...
            steps=[
                ...
                ticket("t1", "Open ATO investigation ticket", "P2", "ATO suspected: {{alert.user}} from {{alert.source_ip}}"),
                close_case(
                    "close-fp", "Close if false positive", condition={"field": "verdict", "operator": "eq", "value": "false_positive"}
                ),
            ],
        ),
        make_playbook(
            ...
            steps=[
                ...
                http(
                    "step-up", "Force step-up auth challenge", url="${IDP_BASE_URL}/users/{{alert.user}}/factors/challenge", method="POST"
                ),
                notify(
                    "n1",
                    "DM user on Slack",
                ...
                http("reset-mfa", "Reset enrolled MFA factors", url="${IDP_BASE_URL}/users/{{alert.user}}/factors", method="DELETE"),
                ticket("t1", "Open MFA-fatigue case", "P3", "MFA fatigue investigation: {{alert.user}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                ...
                http("list", "List impacted users", url="${SIEM_URL}/search/credential_stuffing/{{alert.source_ip}}", method="GET"),
                investigate("inv", "Investigate cred-stuffing campaign", focus="identity"),
                http(
                    "reset-all",
                    "Force reset for impacted users",
        ...
        make_playbook(
            ...
            steps=[
                ...
                http(
                    "revoke", "Revoke all sessions for user", url="${IDP_BASE_URL}/users/{{alert.user}}/sessions/revoke-all", method="POST"
                ),
                investigate("inv", "Investigate token-theft chain", focus="forensics"),
        ...
        make_playbook(
            ...
            steps=[
                enrich("e1", "Enrich OAuth client reputation", "alert.oauth_client_id"),
                investigate("inv", "Investigate scope and lineage", focus="identity"),
                http(
                    "revoke",
                    "Revoke OAuth grant",
    ...
    return [
        make_playbook(
            ...
            steps=[
                ...
                http("snap", "Snapshot host disk", url="${EDR_URL}/hosts/{{alert.host}}/snapshot", method="POST"),
        ...
        make_playbook(
            ...
            steps=[
                isolate_host("iso", "Isolate host (EDR)"),
                http("verify-backup", "Verify last successful backup", url="${BACKUP_URL}/hosts/{{alert.host}}/last-backup", method="GET"),
                investigate("inv", "Forensic investigation", focus="forensics"),
                notify(
                    "n1",
                    "Notify backup team",
        ...
        make_playbook(
            ...
            steps=[
                ...
                http(
                    "smb-deny",
                    "Push deny rule to firewall (SMB)",
        ...
        make_playbook(
            ...
            steps=[
                ...
                investigate("inv", "Investigate ransomware C2", focus="forensics"),
                notify(
                    "n1",
                    "Notify SOC",
                    "slack",
        ...
        make_playbook(
            ...
            steps=[
                http("detach-eip", "Detach public IP", url="${CLOUD_URL}/instances/{{alert.host}}/detach-public-ip", method="POST"),
                isolate_host("iso", "Isolate host (EDR)"),
                http("snap", "Snapshot host disk", url="${EDR_URL}/hosts/{{alert.host}}/snapshot", method="POST"),
                investigate("inv", "Forensic investigation", focus="forensics"),
                notify(
...
def build_bec() -> list[dict[str, Any]]:
    ...
    return [
        make_playbook(
            ...
            steps=[
                enrich("e1", "Enrich rule details", "alert.inbox_rule_id"),
                http(
                    "delete-rule",
                ...
                http("revoke", "Revoke active sessions", url="${IDP_BASE_URL}/users/{{alert.user}}/sessions/revoke-all", method="POST"),
                investigate("inv", "Investigate BEC chain", focus="identity"),
                notify(
                    "n1",
                    "Notify finance team",
                    "slack",
        ...
        make_playbook(
            ...
            steps=[
                http("freeze", "Freeze pending vendor payment", url="${ERP_URL}/payments/{{alert.payment_id}}/freeze", method="POST"),
                ...
                http("release", "Release payment after approval", url="${ERP_URL}/payments/{{alert.payment_id}}/release", method="POST"),
                ticket("t1", "Open BEC payment ticket", "P1", "BEC payment redirect: {{alert.vendor}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                ...
                http(
                    "quarantine",
                    "Quarantine impersonation message",
                ...
                investigate("inv", "Investigate BEC campaign", focus="identity"),
                notify(
                    "n1",
                    "Notify SOC",
                    "slack",
                ...
                ticket("t1", "Open BEC ticket", "P3", "BEC impersonation: {{alert.sender_domain}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                enrich("e1", "Enrich CA policy state", "alert.policy_id"),
                http(
                    "restore", "Restore conditional access policy", url="${IDP_BASE_URL}/policies/{{alert.policy_id}}/enable", method="POST"
                ),
                investigate("inv", "Investigate CA bypass", focus="identity"),
                notify(
                    "n1",
                    "Page identity team",
                    "pagerduty",
                ...
                ticket("t1", "Open CA-bypass ticket", "P2", "CA bypass: {{alert.user}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                enrich("e1", "Enrich ASN reputation", "alert.source_asn"),
                http("revoke-rt", "Revoke refresh tokens", url="${IDP_BASE_URL}/users/{{alert.user}}/refresh-tokens", method="DELETE"),
                http("force-mfa", "Force MFA re-enrollment", url="${IDP_BASE_URL}/users/{{alert.user}}/mfa/reset", method="POST"),
                investigate("inv", "Investigate token-theft chain", focus="identity"),
                notify(
                    "n1",
                    "Page on-call",
                    "pagerduty",
...
def build_insider_risk() -> list[dict[str, Any]]:
    ...
    return [
        make_playbook(
            ...
            steps=[
                enrich("e1", "Enrich user role + tenure", "alert.user"),
                http("readonly", "Set sensitive-store ACL to read-only", url="${SAAS_URL}/acl/{{alert.user}}/readonly", method="POST"),
                investigate("inv", "Investigate access pattern", focus="identity"),
                notify(
                    "n1",
                ...
                ticket("t1", "Open insider-risk ticket", "P3", "Mass download: {{alert.user}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                enrich("e1", "Enrich HR resignation status", "alert.user"),
                http(
                    "readonly",
                    "Force read-only on sensitive scopes",
                    url="${SAAS_URL}/acl/{{alert.user}}/scopes/sensitive/readonly",
                    method="POST",
                ),
                investigate("inv", "Investigate exit-risk pattern", focus="identity"),
                notify(
                    "n1",
                    "Notify HR + manager",
                    "slack",
        ...
        make_playbook(
            ...
            steps=[
                ...
                investigate("inv", "Investigate privilege misuse", focus="identity"),
                notify(
                    "n1",
                    "Page HR-Sec",
                    "pagerduty",
        ...
        make_playbook(
            ...
            steps=[
                ...
                http(
                    "block-dest",
                    "Block destination at proxy",
                    url="${PROXY_URL}/policy/block",
                    method="POST",
                    body_template='{"host":"{{alert.dst_host}}","reason":"src-exfil"}',
                ),
                investigate("inv", "Investigate exfil chain", focus="forensics"),
                notify(
                    "n1",
                    "Page eng-leadership",
                    "pagerduty",
        ...
        make_playbook(
            ...
            steps=[
                enrich("e1", "Enrich user normal pattern", "alert.user"),
                http("step-up", "Force MFA step-up", url="${IDP_BASE_URL}/users/{{alert.user}}/factors/challenge", method="POST"),
                investigate("inv", "Score anomaly likelihood", focus="identity"),
                ticket("t1", "Open after-hours review", "P4", "After-hours access: {{alert.user}}", queue="hr"),
            ],
        ),
    ]
...
def build_cloud_misconfig() -> list[dict[str, Any]]:
    ...
    return [
        make_playbook(
            ...
            steps=[
                http("private", "Set bucket private", url="${AWS_URL}/s3/{{alert.bucket}}/acl/private", method="PUT"),
                http("bpa", "Enable Block Public Access", url="${AWS_URL}/s3/{{alert.bucket}}/block-public-access", method="PUT"),
                investigate("inv", "Audit last 7d S3 access", focus="cloud"),
                notify("n1", "Notify cloud team", "slack", message="S3 {{alert.bucket}} re-privatized.", webhook_env="SLACK_CLOUD_WEBHOOK"),
                ticket("t1", "Open cloud ticket", "P2", "S3 public: {{alert.bucket}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                ...
                investigate("inv", "Investigate access usage", focus="cloud"),
                notify(
                    "n1",
                    "Notify role owner",
                    "slack",
        ...
        make_playbook(
            ...
            steps=[
                http("disable", "Disable leaked key", url="${AWS_URL}/iam/users/{{alert.user}}/keys/{{alert.key_id}}", method="DELETE"),
                http("rotate", "Rotate access key", url="${AWS_URL}/iam/users/{{alert.user}}/keys/rotate", method="POST"),
                investigate("inv", "Find downstream use of key", focus="cloud"),
                notify(
                    "n1",
                    "Page cloud-sec",
                    "pagerduty",
                ...
                ticket("t1", "Open key-leak ticket", "P1", "Leaked key: {{alert.key_id}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                http("reenable-mfa", "Re-enable root MFA", url="${AWS_URL}/iam/root/mfa/enable", method="POST"),
                investigate("inv", "Identify actor and intent", focus="cloud"),
                notify(
                    "n1",
                    "Page platform-owner",
                    "pagerduty",
        ...
        make_playbook(
            ...
            steps=[
                http("start", "Re-enable CloudTrail", url="${AWS_URL}/cloudtrail/{{alert.trail}}/start", method="POST"),
                investigate("inv", "Audit dark-window calls", focus="cloud"),
                notify("n1", "Page cloud-sec", "pagerduty", message="CloudTrail {{alert.trail}} restarted.", service_env="PD_CLOUD_KEY"),
                ticket("t1", "Open CloudTrail ticket", "P1", "CloudTrail disabled: {{alert.trail}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                http("close", "Remove 0.0.0.0/0 ingress rule", url="${AWS_URL}/sg/{{alert.sg}}/ingress/{{alert.rule_id}}", method="DELETE"),
                investigate("inv", "Audit who made the change", focus="cloud"),
                notify(
                    "n1",
                    "Notify cloud team",
                    "slack",
                ...
                ticket("t1", "Open SG ticket", "P2", "SG open: {{alert.sg}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                http("private", "Disable RDS public access", url="${AWS_URL}/rds/{{alert.instance}}/public/disable", method="POST"),
                investigate("inv", "Audit RDS access window", focus="cloud"),
                notify(
                    "n1",
                    "Notify cloud team",
                    "slack",
                ...
                ticket("t1", "Open RDS-public ticket", "P1", "RDS public: {{alert.instance}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                http("disable-anon", "Disable anonymous auth", url="${GCP_URL}/gke/{{alert.cluster}}/anonymous/disable", method="POST"),
                investigate("inv", "Audit anonymous-window calls", focus="cloud"),
                notify(
                    "n1",
                    "Page platform team",
                    "pagerduty",
                ...
                ticket("t1", "Open GKE-anon ticket", "P1", "GKE anonymous: {{alert.cluster}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                http(
                    "private",
                    "Set container private",
                    url="${AZURE_URL}/storage/{{alert.account}}/{{alert.container}}/private",
                    method="POST",
                ),
                investigate("inv", "Audit blob access", focus="cloud"),
                notify(
                    "n1",
                    "Notify cloud team",
                    "slack",
                ...
                ticket("t1", "Open Azure ticket", "P2", "Azure blob public: {{alert.container}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                http(
                    "revoke-trust",
                    "Remove cross-account trust",
                    url="${AWS_URL}/iam/{{alert.role}}/trust/{{alert.external_account}}",
                    method="DELETE",
                ),
                investigate("inv", "Audit assumptions in dark window", focus="cloud"),
                notify(
                    "n1",
                    "Page cloud-sec",
                    "pagerduty",
    ...
    return [
        make_playbook(
            ...
            steps=[
                ...
                investigate("inv", "Investigate exfil pattern", focus="forensics"),
                notify(
                    "n1",
                    "Notify SOC",
                    "slack",
        ...
        make_playbook(
            ...
            steps=[
                enrich("e1", "Enrich domain reputation", "alert.dns_query"),
                http(
                    "sinkhole",
                    "Sinkhole domain at resolver",
                    url="${DNS_URL}/sinkhole",
                    method="POST",
                    body_template='{"domain":"{{alert.dns_query}}"}',
                ),
                isolate_host("iso", "Isolate beaconing host"),
                investigate("inv", "Investigate DNS tunneling", focus="forensics"),
                notify(
                    "n1",
                    "Page on-call",
                    "pagerduty",
        ...
        make_playbook(
            ...
            steps=[
                http("block-usb", "Push USB write-block to host", url="${EDR_URL}/hosts/{{alert.host}}/policy/usb-readonly", method="POST"),
                investigate("inv", "Investigate file sensitivity", focus="forensics"),
                notify(
                    "n1",
                    "Notify user's manager",
                    "email",
    ...
    return [
        ...
        make_playbook(
            ...
            steps=[
                ...
                investigate("inv", "Investigate RDP spray", focus="forensics"),
                notify("n1", "Notify SOC", "slack", message="RDP spray from {{alert.src_host}} blocked.", webhook_env="SLACK_SOC_WEBHOOK"),
                ticket("t1", "Open RDP-spray ticket", "P2", "RDP spray: {{alert.src_host}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                investigate("inv", "Investigate kerberoasting", focus="identity"),
                http(
                    "reset-svc",
                    "Reset impacted SVC accounts",
                    url="${IDP_BASE_URL}/service-accounts/bulk-reset",
                    method="POST",
                    body_template='{"accounts": {{context.impacted_svc_accounts}}}',
                ),
                http("disable-user", "Disable suspect user", url="${IDP_BASE_URL}/users/{{alert.user}}/disable", method="POST"),
                notify(
                    "n1",
                    "Page identity team",
                    "pagerduty",
                ...
                ticket("t1", "Open kerberoast ticket", "P1", "Kerberoasting: {{alert.user}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                http("disable-user", "Disable account", url="${IDP_BASE_URL}/users/{{alert.user}}/disable", method="POST"),
                isolate_host("iso-src", "Isolate source host", "alert.src_host"),
                isolate_host("iso-dst", "Isolate destination host", "alert.dst_host"),
                investigate("inv", "Investigate PTH chain", focus="forensics"),
                notify(
                    "n1",
                    "Page on-call",
                    "pagerduty",
        ...
        make_playbook(
            ...
            steps=[
                ...
                ticket("t1", "Open cross-domain ticket", "P1", "Cross-domain lateral: {{alert.user}}"),
            ],
        ),
    ]
...
def build_supply_chain() -> list[dict[str, Any]]:
    ...
    return [
        make_playbook(
            ...
            steps=[
                enrich("e1", "Enrich package reputation", "alert.package_name"),
                http(
                    "block",
                ...
                investigate("inv", "Investigate supply-chain blast radius", focus="cloud"),
                notify(
                    "n1",
                    "Page eng-sec",
                    "pagerduty",
        ...
        make_playbook(
            ...
            steps=[
                ...
                http(
                    "audit",
                    "List repos with this dependency",
                    url="${SCA_URL}/repos?ecosystem=pypi&package={{alert.package_name}}",
                    method="GET",
                ),
                investigate("inv", "Investigate typosquat", focus="cloud"),
                notify(
                    "n1",
                    "Notify eng team",
                    "slack",
        ...
        make_playbook(
            ...
            steps=[
                ...
                ticket("t1", "Open Action-compromise ticket", "P1", "GH Action compromise: {{alert.action}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                ...
                investigate("inv", "Investigate vendor exposure", focus="cloud"),
                notify(
                    "n1",
        ...
        make_playbook(
            ...
            steps=[
                http(
                    "revert",
                    "Revert to declared IaC state",
                    url="${IAC_URL}/apply",
                    method="POST",
                    body_template='{"resource":"{{alert.resource_id}}"}',
                ),
                investigate("inv", "Investigate drift cause", focus="cloud"),
                notify(
                    "n1",
                ...
                ticket("t1", "Open IaC-drift ticket", "P3", "IaC drift: {{alert.resource_id}}"),
            ],
        ),
    ]
    ...
    return [
        make_playbook(
            ...
            steps=[
                http(
                    "scrub-on",
                    "Engage DDoS scrubbing",
                    url="${DDOS_URL}/scrubbing/enable",
                    method="POST",
                    body_template='{"target":"{{alert.target}}"}',
                ),
                investigate("inv", "Investigate attack signature", focus="forensics"),
                notify(
                    "n1",
                ...
                ticket("t1", "Open DDoS ticket", "P1", "L3/L4 DDoS: {{alert.target}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                ...
                http(
                    "challenge",
                    "Enable challenge page",
                    url="${WAF_URL}/challenge/enable",
                    method="POST",
                    body_template='{"target":"{{alert.target}}"}',
                ),
                investigate("inv", "Investigate L7 signature", focus="forensics"),
                notify(
                    "n1",
                    "Notify SOC + platform",
                    "slack",
                ...
                ticket("t1", "Open L7 DDoS ticket", "P2", "L7 DDoS: {{alert.target}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                http(
                    "sinkhole",
                    "Sinkhole spoofed sources",
                    url="${DNS_URL}/sinkhole-batch",
                    method="POST",
                    body_template='{"src_ips": {{alert.spoofed_sources}}}',
                ),
                investigate("inv", "Investigate amplification", focus="forensics"),
                notify(
                    "n1",
                    "Notify network team",
                    "slack",
        ...
        make_playbook(
            ...
            tags=["ddos", "network", "mitre.t1498"],
            steps=[
                http("syn-cookies", "Enable SYN cookies at edge", url="${EDGE_URL}/syn-cookies/enable", method="POST"),
                http(
                    "scale",
                ...
                notify(
                    "n1",
                    "Notify network on-call",
                    "pagerduty",
                    message="SYN flood on {{alert.target}}. SYN cookies + scale-out.",
                    service_env="PD_NETWORK_KEY",
                ),
                ticket("t1", "Open SYN-flood ticket", "P1", "SYN flood: {{alert.target}}"),
            ],
        ),
        make_playbook(
            ...
            steps=[
                ...
                ticket("t1", "Open auth-DDoS ticket", "P2", "Auth DDoS: {{alert.target}}"),
            ],
        ),
    ]
