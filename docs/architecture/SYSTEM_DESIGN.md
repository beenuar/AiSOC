...
```
                       ...
                       │  HMAC webhooks  │   │  68 connector classes│
...
| `services/connectors` | Python 3.11 | 68 connector classes, APScheduler poll, `push_case` / `push_status_change` for Jira & ServiceNow | CredentialVault decrypt at poll time |
...
A first-class, in-process polling tier with **68 registered connector classes** spanning seven categories — `edr`, `siem`, `cloud`, `iam`, `saas`, `vcs`, `network`. Every connector subclasses `BaseConnector` and declares:
