"""Give the runtime role DML on the osquery-TLS tables, and nothing more.
...
* ``ALTER DEFAULT PRIVILEGES`` is recorded against the role that issued it, so
  ...
  pointing this service at its own database, which ``AISOC_OSQUERY_TLS_DATABASE_URL`` exists
  to allow — gets no default privilege here at all.

The observable failure is a service that starts, connects, and answers every
query with ``permission denied for table osquery_node``. So the chain grants
