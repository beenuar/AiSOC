...
Every other field here is read as ``PURPLE_TEAM_<FIELD>`` because the settings
...
def _clean_env(monkeypatch):
    for key in list(os.environ):
        if key.startswith("PURPLE_TEAM_") or key == "DATABASE_URL":
...
def test_prefixed_env_var_still_works(monkeypatch):
    monkeypatch.setenv("PURPLE_TEAM_DATABASE_URL", "postgresql+asyncpg://legacy:pw@db:5432/legacy")
    from app.core.config import Settings
...
def test_unprefixed_takes_precedence(monkeypatch):
    ...
    monkeypatch.setenv("PURPLE_TEAM_DATABASE_URL", "postgresql+asyncpg://loser:pw@db:5432/lose")
    from app.core.config import Settings
...
def test_other_fields_keep_their_prefix(monkeypatch):
    ...
    monkeypatch.setenv("PURPLE_TEAM_DRIFT_SNAPSHOT_INTERVAL_SECONDS", "7")
    from app.core.config import Settings

    assert Settings().drift_snapshot_interval_seconds == 7
