...
OPERATORS: list[tuple[str, str, str]] = sorted(
    [
        ...
        ("_match", "match", "MATCH"),
        # `neq` is used by rules in the shipped corpus and has never
        # existed here, so `approver_role_neq: "codeowner"` was read as a
...
def _check(field: str, op: str, expected: Any, event: dict[str, Any]) -> bool:
    ...
    if op == "neq":
        # A missing field is not "different from X". Returning True here
        # would make every rule using neq fire on every event that lacks
        # the field, which is a detection that fires on absence of data.
        if actual is None:
            return False
        return actual != expected
