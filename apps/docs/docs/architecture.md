...
- **Conversational investigation chat** — the multi-turn copilot is `CopilotDock` (app-wide) and `/copilot`; `/investigate` permanently redirects to `/hunt`. The separate `InvestigationChat.tsx` component this line used to name was imported by nothing and answered from a canned reply table rather than the API, so it was removed rather than wired.
- **MITRE ATT&CK coverage advisor** — `/coverage-advisor` reads `GET /api/v1/detection/coverage` and ranks the techniques your own rules reference by how much *enabled* coverage each has. It does not rank by adversary prevalence, and techniques no rule references do not appear — the page says so rather than publishing a coverage score it cannot support.
...
- **Connector catalog growth** — SentinelOne, Cortex XDR / XSIAM, Wiz, Snyk, Zscaler, Proofpoint, ServiceNow, Jira, 1Password, Duo Security, Carbon Black, Chronicle, Cisco Umbrella, CrowdStrike, Datadog Cloud SIEM, Elastic, Mimecast, Okta, Rapid7 InsightIDR, Salesforce, Microsoft Sentinel, Splunk, Sumo Logic, Tenable, Trellix Helix, Trend Vision One, and more. **The catalog is now 84 connectors** across EDR, SIEM, cloud, identity, SaaS, network, and email categories. See [Connectors](./connectors/).
