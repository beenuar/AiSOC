...
|-------|-------|------|----------|---------|-------|
| `base_url` | osctrl Admin URL | string | yes | — | Base URL of the osctrl admin server (no trailing slash). Use https in production. |
| `api_token` | API Token | secret | yes | — | **encrypted at rest** · Long-lived admin token issued from the osctrl UI under Manage Users -> generate token. |
| `environment` | Environment Name | string | no | `prod` | osctrl environment to scope queries to (e.g. 'prod', 'corp'). Required for distributed queries. |
