...
|-------|-------|------|----------|---------|-------|
| `api_key` | REST API Key | secret | yes | — | **encrypted at rest** · Generate under Integrations → API Access Keys. Read-only access is sufficient. |
| `subdomain` | Subdomain (optional) | string | no | — | Your acme.pagerduty.com subdomain. Used to build clickable links in normalised events; not required for ingestion. |
