...
|-------|-------|------|----------|---------|-------|
| `account` | Account locator | string | yes | — | Account locator from your Snowflake URL. |
| `user` | Service user | string | yes | — | Snowflake user the JWT identifies. |
| `role` | Role | string | no | — | — |
| `warehouse` | Warehouse | string | no | — | — |
| `private_key_pem` | RSA private key (PKCS#8 PEM) | secret | yes | — | **encrypted at rest** · PKCS#8 PEM-encoded RSA private key paired with the public key registered on the service user. Encrypted at rest by CredentialVault. |

:::note Credential handling
`private_key_pem` is encrypted at rest by the credential vault before the row is written, and decrypted only at poll time. The value is never logged and never returned by the API. See [credential vault](../operations/credentials.md).
:::
