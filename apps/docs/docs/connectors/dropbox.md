...
|-------|-------|------|----------|---------|-------|
| `team_admin_token` | Team admin token | secret | yes | — | **encrypted at rest** · OAuth 2.0 token for a Dropbox Business app with the ``team_data.governance.read`` and ``events.read`` scopes. |

:::note Credential handling
`team_admin_token` is encrypted at rest by the credential vault before the row is written, and decrypted only at poll time. The value is never logged and never returned by the API. See [credential vault](../operations/credentials.md).
:::
