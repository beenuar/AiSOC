...
|-------|-------|------|----------|---------|-------|
| `account_id` | Account ID | string | yes | — | Found in the Cloudflare dashboard under any zone's Overview, lower-right pane ("Account ID"). |
| `api_token` | API Token | secret | yes | — | **encrypted at rest** · Account-scoped API token with the Audit Logs: Read permission. Avoid using the legacy Global API Key. |
