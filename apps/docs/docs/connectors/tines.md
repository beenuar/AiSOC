...
|-------|-------|------|----------|---------|-------|
| `base_url` | Tines tenant URL | string | yes | — | Your Tines tenant URL, including scheme. For self-hosted deployments use the full HTTPS URL of the Rails app. |
| `api_token` | Personal Access Token | secret | yes | — | **encrypted at rest** · Generate under Profile → Personal Access Tokens. Read-only scope is sufficient; the connector never writes back to Tines. |
| `team_id` | Team ID (optional) | string | no | — | Scope event ingestion to a single Team. Leave blank to ingest across all teams the token can see. |
