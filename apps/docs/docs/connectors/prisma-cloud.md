...
|-------|-------|------|----------|---------|-------|
| `api_url` | API URL | string | yes | — | Region-specific base URL of the Prisma Cloud REST API. See Prisma Cloud → System → API Endpoints for the value for your tenant (e.g. ``api2.eu``, ``api.gov``). |
| `access_key_id` | Access Key ID | string | yes | — | — |
| `secret_key` | Secret Key | secret | yes | — | **encrypted at rest** |
| `compute_url` | Compute API URL (optional) | string | no | — | Tenant-specific Compute (Twistlock) API base URL. Leave blank — v7.1.0 only consumes the unified ``/alert`` endpoint. |
