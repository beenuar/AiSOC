...
|-------|-------|------|----------|---------|-------|
| `organization_id` | Organization ID | string | yes | — | Numeric GCP organization ID. SCC is org-scoped; the service account must have findingsViewer at this level (or on a parent folder/source). |
| `service_account_json` | Service account JSON key | secret | yes | — | **encrypted at rest** · Paste the full JSON key file. It will be encrypted at rest by the credential vault. |
