...
|-------|-------|------|----------|---------|-------|
| `webhook_path` | Webhook path | string | yes | `/v1/webhooks/falco` | Path on the AiSOC ingest service where Falco / falcosidekick should POST events. |
| `shared_secret` | Shared secret (optional) | secret | no | — | **encrypted at rest** · If set, each incoming request must include the matching value in the X-Falco-Secret header. |
| `minimum_priority` | Minimum priority | select | no | `DEBUG` | Drop events whose Falco priority is below this threshold. Defaults to DEBUG so nothing is filtered server-side. · one of: `DEBUG`, `INFORMATIONAL`, `NOTICE`, `WARNING`, `ERROR`, `CRITICAL`, `ALERT`, `EMERGENCY` |

:::note Credential handling
`shared_secret` is encrypted at rest by the credential vault before the row is written, and decrypted only at poll time. The value is never logged and never returned by the API. See [credential vault](../operations/credentials.md).
:::
