...
|-------|-------|------|----------|---------|-------|
| `vault_addr` | Vault address | string | no | — | Cluster URL used by test_connection. Can be left blank to skip the live probe. |
| `vault_token` | Vault token (audit-list policy) | secret | no | — | **encrypted at rest** · Token with sys/audit list / sys/health read. Used only by test_connection — events flow via the audit-device sidecar, not through this token. |
| `namespace` | Vault Enterprise namespace | string | no | — | Optional. Sent as X-Vault-Namespace. |
| `buffer_size` | Max buffered audit lines | string | no | `5000` | — |

:::note Credential handling
`vault_token` is encrypted at rest by the credential vault before the row is written, and decrypted only at poll time. The value is never logged and never returned by the API. See [credential vault](../operations/credentials.md).
:::
