...
|-------|-------|------|----------|---------|-------|
| `tailnet` | Tailnet | string | yes | — | Your tailnet name (the organisation slug shown in the Tailscale admin console, e.g. 'example.com'). Use '-' to use the default tailnet associated with the API key / OAuth token. |
| `api_key` | API Key (optional if using OAuth) | secret | no | — | **encrypted at rest** · Tailscale API key (starts with 'tskey-api-'). Use either this *or* the OAuth client credentials below — not both. |
| `client_id` | OAuth Client ID (optional) | string | no | — | OAuth 2.0 client ID from the Tailscale admin console. Required scope: 'audit:read'. |
| `client_secret` | OAuth Client Secret (optional) | secret | no | — | **encrypted at rest** · OAuth 2.0 client secret. Only needed when using client-credential flow instead of an API key. |

:::note Credential handling
`api_key`, `client_secret` are encrypted at rest by the credential vault before the row is written, and decrypted only at poll time. The value is never logged and never returned by the API. See [credential vault](../operations/credentials.md).
:::
