...
not real bills. The two row types share the same columns so the table reads
uniformly, but **never quote a substrate row as live agent performance**.

**There are no wet-eval rows yet.** The
[T5.5 weekly job](./benchmark-methodology.md#3-substrate-vs-wet-eval) has
landed and runs every Monday, but this repository has no `WET_EVAL_OPENAI_KEY`
configured, so every run exits cleanly at the credential gate without calling a
model. Until a funded key is in place, every row you see below is a substrate
row. We would rather publish an empty wet-eval history than fill it with
projections relabelled as measurements.
:::
...
2. **Wet-eval rows (T5.5 weekly job).** The
   ...
   on a Monday 07:00 UTC cadence, captures real latency / token / USD telemetry,
