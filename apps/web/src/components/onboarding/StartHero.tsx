...
export function StartHero() {
  ...
  const handleTryDemo = async () => {
    ...
    const email = process.env.NEXT_PUBLIC_DEMO_AUTOLOGIN_EMAIL?.trim() || 'demo@example.com';
 ...
 * primary "Try the demo" path.
 *
 * This card used to link `/demo.mp4`, a text placeholder, under the label
 * "Coming v8.0" — a 404 wearing a promise that went stale three majors
 * before anyone noticed, on a surface the "no dead controls" rule covers.
 * It now points at the recording that exists: the deployment walkthrough at
 * `/demo/demo.mp4`, described in `apps/web/public/demo/README.md`. The copy
 * describes that recording rather than the four-case product tour, which is
 * still unrecorded (brief in `apps/web/public/.demo-mp4-placeholder`).
 *
 * Why an `<a>` rather than an inline `<video>`? It keeps the bundle free of
 * media controls, autoplay mute-gating and caption plumbing on a card that
 * is deliberately secondary to the three CTAs above it. The thumbnail is a
 * real frame from the recording, so it cannot drift from the content the
 * way the hand-drawn SVG it replaced had.
 */
function DemoScreencast() {
  return (
    <aside
      aria-label="AiSOC deployment walkthrough"
    ...
    >
      ...
      >
        {/* Thumbnail. <img> rather than next/image so the frame ships as-is
        ...
        <span className="relative block w-full overflow-hidden rounded-lg border border-white/10 sm:w-56 sm:flex-none">
          ...
          <img
            src="/demo/demo-poster.png"
            alt="A frame from the walkthrough: make up finishing, reporting every service healthy and printing the address to sign in at"
            width={1280}
            height={720}
        ...
        <span className="flex flex-1 flex-col gap-1">
          <span className="inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wider text-brand-300">
            ...
            <span className="ml-2 rounded-full border border-white/15 bg-white/5 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-gray-300">
              2 min 57 s
            </span>
          </span>
          <span className="text-base font-semibold text-white">
            One host, install to AI verdict
          </span>
          <span className="text-xs leading-relaxed text-gray-400">
            <span className="font-mono">make up</span> against the published images,
            real CISA KEV data, one event pushed through to an alert and a triage
            verdict with its token count. Nothing seeded; terminal waits shortened
            and labelled.
          </span>
        </span>
      </a>
