...
[Try in Codespaces](https://codespaces.new/beenuar/AiSOC?quickstart=1) · [Live demo](https://tryaisoc.com) · [Quickstart](#try-aisoc-in-60-seconds) · [Architecture](#architecture) · [Docs](apps/docs/) · [Discussions](https://github.com/beenuar/AiSOC/discussions) · [Release notes](RELEASES.md)
...
`VERSION` is `7.5.0` (2026-06-29) — a v8.0-milestone and trust-readiness release that tags the `AiSOC missing pieces — Phases 1–5` rollup ([PR #337](https://github.com/beenuar/AiSOC/pull/337)), the four named v8.0 milestones (T3.7 NL→playbook, T3.8 design system v2 + Storybook, T4 wave-3 marketplace + 6 hardened connectors, T5.3 fidelity loaders), threat-actor attribution port fix + optional RBAC, realtime short-lived-ticket auth, boundary-aware KB chunker, Terraform CI, and a large Dependabot + security sweep.

- **Latest GitHub release with downloads:** <https://github.com/beenuar/AiSOC/releases/latest>
- **Per-release narrative (mirrors what used to live here):** [`RELEASES.md`](RELEASES.md)
...
<details>
<summary><strong>Click for the highlight reel that used to live here (v7.5.0 → v7.4.0 → v1.5 → architectural foundation → security & correctness wave).</strong> The full per-release narrative is now maintained in <a href="RELEASES.md"><code>RELEASES.md</code></a> so the front page stays scannable.</summary>

The "What's new" section that used to live on this README in full is now in [`RELEASES.md`](RELEASES.md) so this page stays focused on "what is AiSOC and how do I run it." Open [`RELEASES.md`](RELEASES.md) for the full v7.5.0 → v7.4.0 → v1.5 narrative.
...
The public roadmap lives in [ROADMAP.md](ROADMAP.md). All releases through **v7.5.0** have shipped, including:

- **v7.5.0** — v8.0-milestone and trust-readiness release (2026-06-29): the `AiSOC missing pieces — Phases 1–5` rollup ([PR #337](https://github.com/beenuar/AiSOC/pull/337); 25 commits, 188 files, +23 743 / -907); the four named v8.0 milestones (T3.7 NL→playbook, T3.8 design system v2 + Storybook, T4 wave-3 marketplace + 6 hardened connectors, T5.3 AIT-LDS + MITRE Engenuity fidelity loaders); the marketing-shell unification on `tryaisoc.com` (single `StickyNav` + `sections/Footer` shell across `(marketing)/*`, `/not-found`, `/why-open-source`, `/benchmark`); the threat-actor attribution port fix + optional shared-secret RBAC on `/api/v1/actors/*`; the realtime short-lived-ticket auth on WS/SSE; the boundary-aware KB ingest chunker; the Terraform CI workflow + the three missing reusable infra modules (`rds`, `elasticache`, `kafka`); the deterministic NL → ES|QL / KQL / SPL translator at **81-case eval, 100 % syntactic + 100 % semantic**; and a large Dependabot + security sweep (`aiohttp` CVE-2026-34993 + CVE-2026-47265 cleared, pnpm audit high/critical findings cleared, FastAPI / `next` / `framer-motion` / `cryptography` updates across services).
...
- **Stage 2 / Stage 3 platform additions** — shipped in v7.4.0 alongside the architectural foundation below:
  - **Wazuh Indexer ingest connector** + auditd `file_tail` connector + `aisoc.rules` audit profile (replaces the host-agent dependency for Linux endpoint visibility). The connector registry now declares **69 first-party connectors**.
